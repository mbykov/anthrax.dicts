...
// import { prettyName } from "@mbykov/anthrax/prettyFLS"
...
export async function wktNoun(nests, nounPath, only) {
    ...
    for await (let fname of fns) {
        ...
        // log('________________________________________________NEST', nest)
...
function parseNoun(entry, nest) {
    ...
    if (nest.zero) cdict.znom = true

    entry.dialects.forEach(dia=> {
        parseDialect(cdict, dia)
    })
    ...
    if (!nest.zero) return
    delete cdict.zero // fls - только соотв. стемам
    ...
    if (nest.prefix) zdict.prefix = nest.prefix
    else if (!nest.prefix) zdict.aug = nest.aug
    nouns.push(zdict)
}
...
function parseDialect(cdict, dia) {
    ...
    for (let form of dia.forms) {
        ...
        let cwf = comb(wf)
        let stem = cdict.stem
        if (cdict.zero) stem = cdict.zstem
        let term = termByStem(cwf, cdict.stem)
        // log('_yyyyyyyyyyyyyy', wf, cdict.stem, term)
        if (term == plain(cwf)) continue

        if (cdict.znom) {
            let rezstem = new RegExp(cdict.zstem)
            if (rezstem.test(plain(cwf))) continue
        }
    ...
    for (let gend in gendgdoups) {
        ...
        let stype = [cdict.type, gen].join('-') // в zero нет nom, беру n_est.type всегда
